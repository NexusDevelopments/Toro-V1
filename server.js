...
const devLinksHtml = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Dev Links</title><style>*{box-sizing:border-box;margin:0;padding:0}body{background:#090304;color:#f4d4d8;font-family:ui-sans-serif,system-ui,sans-serif;min-height:100vh;padding:24px}.wrap{max-width:960px;margin:0 auto}.card{background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(10px);border-radius:16px;padding:18px;margin-bottom:16px}h1{font-size:2rem;color:#ff7788;margin-bottom:8px}h2{font-size:1.04rem;margin-bottom:10px;color:#ffc7cf}.muted{opacity:.7;font-size:.84rem;line-height:1.45}input,select{width:100%;padding:10px 12px;border-radius:999px;background:#130709;border:1px solid rgba(255,255,255,.2);color:#fff;outline:none;appearance:none}select{cursor:pointer}button{background:linear-gradient(135deg,rgba(255,255,255,.15),rgba(255,255,255,.06));border:1px solid rgba(255,255,255,.26);color:#ffecef;border-radius:999px;padding:9px 14px;cursor:pointer}button:hover{border-color:rgba(255,255,255,.45)}.row{display:flex;gap:10px;flex-wrap:wrap}.hidden{display:none}.link{padding:10px;border:1px solid rgba(255,255,255,.14);border-radius:12px;background:rgba(0,0,0,.26)}.line{display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap}.url{font-size:.86rem;word-break:break-all}.warn{margin-top:8px;color:#ffb8c0;font-size:.8rem}.ideas{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;margin-top:12px}.idea{border-radius:12px;padding:10px 12px;text-align:left}.notice{margin-top:10px;padding:10px 12px;border:1px solid rgba(255,255,255,.16);border-radius:12px;background:rgba(255,184,192,.08);font-size:.82rem;color:#ffb8c0}.providers{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.pcard{border:1px solid rgba(255,255,255,.14);border-radius:12px;padding:10px 12px;cursor:pointer;transition:border-color .15s}.pcard.selected{border-color:#ff7788;background:rgba(255,119,136,.08)}.pcard .ptitle{font-size:.92rem;color:#ffc7cf;margin-bottom:3px}.pcard .pdomain{font-size:.78rem;opacity:.6}.pcard .pstatus{font-size:.75rem;margin-top:4px}</style></head><body><div class="wrap"><h1>Dev Links</h1><p class="muted">Create temporary CDN/tunnel links using your preferred provider. Multiple links are supported.</p><div id="auth" class="card"><h2>Authenticate</h2><input id="pw" type="password" placeholder="Admin password"/><div style="height:10px"></div><button id="login">Enter</button><p id="err" class="warn" style="display:none"></p></div><div id="panel" class="hidden"><div class="card"><h2>Create Link</h2><div class="row"><input id="target" value="https://torov1.up.railway.app"/></div><div style="height:10px"></div><h2>Provider</h2><div id="providers" class="providers"></div><div style="height:10px"></div><div class="row"><input id="term" placeholder="Enter a theme like education, gaming, health…"/></div><div style="height:8px"></div><div class="row"><button id="generate">Generate Site Names</button></div><div id="ideas" class="ideas"></div><div style="height:10px"></div><div class="row"><input id="sub" placeholder="Site / zone name (used as identifier)"/></div><p class="muted" style="margin-top:6px">For BunnyCDN this becomes the pull zone name (e.g. <code>myzone</code> → myzone.b-cdn.net). For Cloudflare the tunnel URL is randomly assigned.</p><div style="height:10px"></div><div class="row"><button id="create">Create Link</button></div><div id="create-note" style="display:none;margin-top:10px;padding:10px 12px;border:1px solid rgba(255,184,100,.3);border-radius:12px;background:rgba(255,184,100,.08);font-size:.82rem;color:#ffd580;line-height:1.5"></div></div><div class="card"><h2>Active / Saved Links</h2><div id="links" style="display:grid;gap:8px"></div></div></div></div><script>let PASS='';let providers={};let selectedProvider='cloudflared';function esc(s){return String(s||'').replace(/[&<>\"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[m]||m));}async function post(url,data){const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});const j=await r.json().catch(()=>({}));if(!r.ok) throw new Error(j.error||('Request failed '+r.status));return j;}function setErr(t){const e=document.getElementById('err');if(!t){e.style.display='none';return;}e.style.display='block';e.textContent=t;}const PROVIDER_META={cloudflared:{title:'Cloudflare Tunnel',domain:'*.trycloudflare.com',note:'Random subdomain assigned automatically. No account needed.'},bunnycdn:{title:'BunnyCDN Pull Zone',domain:'*.b-cdn.net',note:'Requires BUNNYCDN_API_KEY env var. You choose the zone name.'}};function paintProviders(){const box=document.getElementById('providers');box.innerHTML='';Object.entries(PROVIDER_META).forEach(([key,meta])=>{const info=providers[key]||{available:false,reason:''};const card=document.createElement('div');card.className='pcard'+(key===selectedProvider?' selected':'');card.innerHTML='<div class="ptitle">'+esc(meta.title)+'</div><div class="pdomain">'+esc(meta.domain)+'</div><div class="pstatus" style="color:'+(info.available?'#a0ffb8':'#ffb8c0')+'">'+esc(info.available?'Available':info.reason||'Unavailable')+'</div>';card.onclick=()=>{selectedProvider=key;paintProviders();};box.appendChild(card);});}function paintIdeas(items){const box=document.getElementById('ideas');box.innerHTML='';(items||[]).forEach(item=>{const button=document.createElement('button');button.className='idea';button.type='button';button.textContent=item.label;button.onclick=()=>{document.getElementById('sub').value=item.label;};box.appendChild(button);});if((items||[]).length===0){box.innerHTML='<p class="muted">No suggestions yet.</p>';}}async function generateIdeas(){try{const term=document.getElementById('term').value.trim();if(!term){paintIdeas([]);return;}const data=await post('/dev/api/links/suggest-names',{password:PASS,term});paintIdeas(data.suggestions||[]);}catch(e){alert(e.message||'Failed to generate site names');}}async function refresh(){const data=await post('/dev/api/links/list',{password:PASS});providers=data.providers||{};paintProviders();const box=document.getElementById('links');box.innerHTML='';(data.links||[]).forEach(l=>{const d=document.createElement('div');d.className='link';const providerMeta=PROVIDER_META[l.provider]||{title:l.provider||'cloudflared',domain:''};const badge='<span style="font-size:.73rem;opacity:.55;margin-left:6px">'+esc(providerMeta.title)+'</span>';const savedName=l.requestedSubdomain?'<div class="muted">name: '+esc(l.requestedSubdomain)+'</div>':'';d.innerHTML='<div class="line"><div><div class="url"><a href="'+esc(l.url)+'" target="_blank" rel="noreferrer">'+esc(l.url)+'</a>'+badge+'</div><div class="muted">status: '+esc(l.status||'unknown')+' • '+new Date(l.createdAt).toLocaleString()+'</div>'+savedName+'</div><button data-id="'+esc(l.id)+'">Stop / Delete</button></div>';d.querySelector('button').onclick=async()=>{try{await post('/dev/api/links/stop',{password:PASS,id:l.id});await refresh();}catch(e){alert(e.message||'Failed');}};box.appendChild(d);});if((data.links||[]).length===0){box.innerHTML='<p class="muted">No links yet.</p>';}}document.getElementById('login').onclick=async()=>{setErr('');try{PASS=document.getElementById('pw').value||'';await post('/dev/api/login',{password:PASS});document.getElementById('auth').classList.add('hidden');document.getElementById('panel').classList.remove('hidden');await refresh();}catch(e){setErr(e.message||'Auth failed');}};document.getElementById('generate').onclick=generateIdeas;document.getElementById('term').addEventListener('keydown',e=>{if(e.key==='Enter') generateIdeas();});document.getElementById('create').onclick=async()=>{const noteBox=document.getElementById('create-note');noteBox.style.display='none';noteBox.textContent='';try{const target=document.getElementById('target').value.trim();const desiredSubdomain=document.getElementById('sub').value.trim();const result=await post('/dev/api/links/create',{password:PASS,target,desiredSubdomain,provider:selectedProvider});if(result.note){noteBox.textContent=result.note;noteBox.style.display='block';}await refresh();}catch(e){alert(e.message||'Failed to create link');}};</script></body></html>`;
